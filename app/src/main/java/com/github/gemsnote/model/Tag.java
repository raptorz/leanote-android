...
public class Tag extends BaseModel{
    ...
    public static Tag getByTagId(String tagId, String userId) {
        return SQLite.select()
                .from(Tag.class)
                .where(Tag_Table.userId.eq(userId))
                .and(Tag_Table.tagId.eq(tagId))
                .querySingle();
    }

    public static void deleteTagAndRelations(Tag tag) {
        SQLite.delete()
                .from(RelationshipOfNoteTag.class)
                .where(RelationshipOfNoteTag_Table.tagLocalId.eq(tag.getId()))
                .execute();
        tag.delete();
    }
